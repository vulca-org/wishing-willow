...
| 自动展开 | **理解到达**或 **Claude 发问**（选择题、批准计划）时原地展开几秒再收回。没有新东西时两翼缩回刘海 |
| 悬停 | 展开：你的要求 / Claude 的理解 / Claude 在做（人话步骤 + 分段进度条）+ 上下文、缓存、输出、写了理解几轮 |
| 点击 | 面板：各会话，与这个会话最近 20 轮（逐轮可展开，附各轮时长图） |

自动展开挂在「理解到达」上，而不是回车：回车那一刻还什么都没有，展开只能摆一行原话；
...
![展开态](../docs/media/expanded.zh.png)
...
`scripts/readme-media.sh` 用插件自己的 `capture.mjs` 建虚构会话、逐行追加聊天记录，在 `--backdrop` 上拍中英两套静帧和展开动图，
输出到 `docs/media/`。需要屏幕录制权限，屏幕要解锁。
